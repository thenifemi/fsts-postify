  ...
  try {
    ...
    if (existingDislike) {
      ...
      return NextResponse.json(
        { disliked: false, message: 'Comment undisliked successfully' },
        { status: 200 }
      );
    } else {
      ...
      return NextResponse.json(
        { disliked: true, message: 'Comment disliked successfully' },
        { status: 200 }
      );
    }
  } catch (error) {
    console.error(
      `Error disliking/undisliking comment ${params.commentId}:`,
      error
    );
    return NextResponse.json(
